...
def insert_score(db_filename, score, countries_list, wrong_countries):
    ...
    cursor.execute("INSERT INTO scores (score, timestamp, game_sequence, mistakes) VALUES (?, ?, ?, ?)", (score, timestamp, countries, mistakes))
    connection.commit()
    connection.close()
...
def get_top_scores(db_filename, limit=10):
    connection = sqlite3.connect(db_filename)
    cursor = connection.cursor()
    cursor.execute("SELECT score, timestamp, game_sequence, mistakes FROM scores ORDER BY score DESC LIMIT ?", (limit,))
